...
public class TaskView extends AbstractStyledView<StackPane> {
	...
	protected void createWidgets() {
		...
		this.lblRemainingDays = new Label("");
		this.lblAttachments = new Label("", ImageLibrary.getImageView("attachments.png"));

		this.boxButton = new HBox(10);
		this.btnSubmit = new JFXButton(Language.getString("ReviewAndSubmit"),
				ImageLibrary.getImageView("check_next.png"));
		this.btnFinish = new JFXButton("", ImageLibrary.getImageView("finished.png"));
		this.btnRedo = new JFXButton(Language.getString("ReOpen"),
				ImageLibrary.getImageView("repeat.png"));
	...
	protected void setupInteractions() {
		...
		this.root.setOnMouseClicked(
				e -> new TaskDialog(this.parent, this.module.get(), this.task).show());

		this.btnSubmit.setOnAction(
				e -> new TaskSubmitDialog(this.parent, this.module.get(), this.task).show());
	...
	protected void addWidgets() {
		...
		this.taskBox.getChildren().add(this.lblRemainingDays);
		this.taskBox.getChildren().add(this.lblAttachments);
		this.taskBox.getChildren().add(LayoutUtils.getVSpacer());

		this.boxButton.getChildren().addAll(this.btnFinish, LayoutUtils.getHSpacer(),
				this.btnSubmit);
	...
	private class TaskListCell extends ListCell<Task> implements ChangeListener<Boolean> {
		...
		private void setupButtonsAndColoring() {
			if (TaskView.this.task.isFinished()) {
				...
				TaskView.this.boxButton.getChildren().addAll(TaskView.this.btnDelete,
						TaskView.this.btnRedo);
				TaskView.this.taskBox.pseudoClassStateChanged(finished, true);
			} else {
				TaskView.this.boxButton.getChildren().addAll(TaskView.this.btnFinish,
						TaskView.this.btnSubmit);
				...
				if (TaskView.this.task.getDueDate() == null) {
					...
					if (LocalDate.now().isAfter(TaskView.this.task.getDueDate().minusDays(2))) {
						TaskView.this.taskBox.pseudoClassStateChanged(noTimeLeft, true);
							...
							.isAfter(TaskView.this.task.getDueDate().minusDays(4))) {
		...
		private void setupTextComponents() {
			...
			TaskView.this.lblDueDate
					.textProperty().bind(
							Bindings.when(TaskView.this.task.dueDateProperty().isNull()).then("-")
									.otherwise(
			...
			TaskView.this.lblRemainingDays.textProperty()
					.bind(Bindings.when(TaskView.this.task.dueDateProperty().isNull()).then("oo")
							.otherwise(Bindings.createStringBinding(() -> {
								long remainingDays = LocalDate.now().until(task.getDueDate(),
										ChronoUnit.DAYS);
								String remainingDaysHint = "";
								if (remainingDays == 0) {
									remainingDaysHint = "heute";
								} else if (remainingDays < 0) {
									remainingDaysHint = "vor " + Math.abs(remainingDays)
											+ " Tag(en)";
								} else if (remainingDays > 0) {
									remainingDaysHint = "noch " + remainingDays + " Tag(e)";
								}
								return remainingDaysHint;
							}, task.dueDateProperty())));
			TaskView.this.lblAttachments.textProperty()
					.bind(TaskView.this.task.attachmentsProperty().sizeProperty().asString());
		}
